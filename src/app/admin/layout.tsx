  ...
  return (
    <div className="flex flex-col h-full bg-[#1e293b] text-slate-300 font-body">
      <div className="p-6 flex items-center gap-3 border-b border-slate-700/50">
        ...
        {(isSidebarOpen || isMobile) && (
          <div className="overflow-hidden whitespace-nowrap">
            <h1 className="text-sm font-black text-white">北海学園大学新聞</h1>
          </div>
        )}
      </div>
      ...
      <nav className="flex-1 p-4 space-y-1.5 mt-4 overflow-y-auto">
        {MENU_ITEMS.map((item) => (
          <Link key={item.id} href={item.id} onClick={onCloseMobile}>
            <button className={cn("w-full flex items-center gap-3 px-3 py-3 rounded-xl transition-all group", pathname === item.id ? "bg-primary text-white" : "hover:bg-slate-800 hover:text-white")}>
              <item.icon className={cn("h-5 w-5", pathname === item.id ? "text-white" : "text-slate-400 group-hover:text-white")} />
              {(isSidebarOpen || isMobile) && <span className="text-sm font-bold">{item.label}</span>}
            </button>
          </Link>
        ))}
      </nav>
      ...
      <div className="p-4 border-t border-slate-700/50">
        <Button variant="ghost" className="w-full text-slate-400 hover:text-white hover:bg-slate-800 justify-start gap-3 rounded-xl h-12" onClick={onLogout}>
...
export default function AdminLayout({ children }: { children: React.ReactNode }) {
  ...
  if (!user) return <div className="min-h-screen flex items-center justify-center p-4 bg-[#F0F2F5]"><div className="w-full max-w-md"><LoginForm /></div></div>;
  ...
  return (
    <div className="min-h-screen bg-slate-50 flex font-body overflow-x-hidden">
      {!isMobile && (
        <aside className={cn("bg-[#1e293b] transition-all duration-300 flex flex-col fixed inset-y-0 z-50", isSidebarOpen ? "w-64" : "w-20")}>
          <SidebarContent pathname={pathname} isSidebarOpen={isSidebarOpen} onLogout={handleLogoutAction} isMobile={false} />
        </aside>
      )}
      ...
      {isMobile && (
        <Sheet open={isMobileMenuOpen} onOpenChange={setIsMobileMenuOpen}>
          <SheetContent side="left" className="p-0 w-72 bg-[#1e293b] border-none">
            <SidebarContent pathname={pathname} isSidebarOpen={true} isMobile={true} onLogout={handleLogoutAction} onCloseMobile={() => setIsMobileMenuOpen(false)} />
          </SheetContent>
        </Sheet>
      )}

      <div className={cn("flex-1 flex flex-col transition-all duration-300 min-w-0", !isMobile ? (isSidebarOpen ? "ml-64" : "ml-20") : "ml-0")}>
        <header className="h-16 bg-white border-b px-4 md:px-8 flex items-center justify-between sticky top-0 z-40">
          <div className="flex items-center gap-4">
            <Button variant="ghost" size="icon" onClick={() => isMobile ? setIsMobileMenuOpen(true) : setIsSidebarOpen(!isSidebarOpen)} className="text-slate-500"><Menu className="h-5 w-5" /></Button>
